...
export const ForgetPassword = () => {
  ...
  const handleResetEmailRequest = async (values: Omit<LoginUserCredentials, 'password'>, actions: FormikHelpers<Omit<LoginUserCredentials, 'password'>>) => {
    const { email } = values;
    requestEmailResetCall(email).then(res => {
      setStep(2)
      setEmailState(email)
    }).catch(err => triggerGenericNotification("An error occurred!", err, "danger"))
  }
      ...
      {step === 1 &&
        <div className='flex flex-col gap-8 pl-[21%] pr-[21%] mt-[8%]'>
          <div className='flex flex-col'>
            ...
            <p className="text-xl font-normal text-darkGrey-600 whitespace-nowrap">No worries, we'll send you a link to reset it.</p>
          </div>
          <Formik
            initialValues={{
              email: '',
          ...
          <span className='self-center flex flex-row gap-3 items-center font-medium hover:underline hover:cursor-pointer' onClick={() => navigate('/login')}><BiArrowBack />Back to Login</span>
        </div>
      }
      ...
      {step === 2 &&
        <div className='flex flex-col gap-8 pl-[21%] pr-[21%] mt-[8%]'>
          <div className='flex flex-col'>
            ...
            <p className="text-xl font-normal text-darkGrey-600 whitespace-nowrap">We sent a reset link to <br /> <span className="font-medium">{emailState}</span></p>
          </div>
          <span className='self-start flex flex-row gap-3 items-center font-medium'>Didn't receive the email?<span className="underline font-bold hover:cursor-pointer" onClick={() => setStep(1)}>Click to resend.</span></span>
        </div>}
    </div>
